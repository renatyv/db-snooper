...
For each table:
1. Skip empty tables. A table with zero rows carries no data context, so it is excluded from the profile by default. The skipped names are listed once in a trailing summary bullet, e.g. `- Skipped 2 empty table(s): foo, bar`. `ProfileOptions(include_empty_tables=True)` forces their inclusion; an included empty table emits only its `CREATE TABLE` DDL (and an empty `all rows` marker), with no rows or column profiles (there is nothing to profile).
